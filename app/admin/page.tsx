...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-10">
      <div>
        <h1 className="text-3xl font-black tracking-tighter text-black mb-2 uppercase">ダッシュボード</h1>
        <p className="text-black">こんにちは。現在のプラットフォームの状況を確認できます。</p>
      </div>
        ...
        {stats.map((stat, i) => (
          <div key={i} className="glass-panel p-6 border-l-4 border-l-[var(--color-brand-blue)]">
            <div className="flex items-center justify-between mb-4">
              <div className={`p-2 bg-zinc-100 rounded-lg ${stat.color}`}>
                {stat.icon}
              </div>
              <span className="text-[10px] font-bold text-black uppercase tracking-widest flex items-center gap-1">
                <TrendingUp size={12} className="text-green-600" /> +0%
              </span>
            </div>
            <p className="text-3xl font-black text-black mb-1">{stat.value}</p>
            <p className="text-xs font-bold text-black uppercase tracking-widest">{stat.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Quick Tips */}
        <section className="glass-panel p-8 bg-blue-50 border border-blue-200">
