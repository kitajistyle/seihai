    ...
    <section className="max-w-7xl mx-auto px-4 py-20">
      ...
      <div className="glass-panel overflow-hidden">
        <table className="w-full text-left whitespace-nowrap md:table block">
          <thead className="bg-zinc-100 text-black text-xs uppercase tracking-wider hidden md:table-header-group">
            <tr>
          ...
          <tbody className="divide-y divide-zinc-200/60 flex flex-col md:table-row-group">
            {rankings.slice(0, 5).map((player) => (
              <tr key={player.id} className="hover:bg-zinc-50 transition-colors group flex items-center p-4 md:p-0 md:table-row gap-4 md:gap-0">
                <td className="md:px-6 md:py-4 font-mono text-black font-bold text-xl md:text-base w-8 md:w-auto text-center md:text-left">{player.rank}</td>
                <td className="md:px-6 md:py-4 flex-1 overflow-hidden">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col gap-1 items-start min-w-0">
                      <span className="font-bold text-base md:text-base text-black truncate w-full">{player.name}</span>
                      {player.x_id && (
                        <a 
                          href={`https://x.com/${player.x_id}`} 
                          target="_blank" 
                          rel="noopener noreferrer" 
                          className="inline-flex items-center gap-1.5 px-2 py-1 md:px-3 md:py-1.5 bg-zinc-100 hover:bg-zinc-200 border border-zinc-250 rounded-lg text-[10px] md:text-xs font-bold text-black hover:text-black transition-all w-fit"
                        >
                ...
                <td className="md:px-6 md:py-4 font-mono text-lg font-bold text-black text-right shrink-0">{player.points}<span className="md:hidden text-xs text-zinc-500 ml-1 font-sans font-normal block">PT</span></td>
              </tr>
            ))}
          </tbody>
        </table>
