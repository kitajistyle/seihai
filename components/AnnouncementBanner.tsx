    ...
    <div className="bg-[#0a0a0c] border-b border-white/5 py-3 px-4">
      <div className="max-w-7xl mx-auto space-y-2">
        {announcements.map((a) => {
          ...
          const inner = (
            <div className={`flex items-center gap-3 rounded-xl border px-4 py-2.5 text-sm ${rowStyle} ${isClickable ? 'hover:brightness-110 transition-all' : ''}`}>
              ...
              {isClickable && <ChevronRight size={14} className="shrink-0 opacity-60" />}
            </div>
          );
